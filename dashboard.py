...
# Function to plot charts in two-column layout
def show_plot(title, fig):
    st.markdown(f"#### {title}")
    st.pyplot(fig)
    st.markdown("---")
...
with col1:
    ...
    show_plot("1. Total Transaction Amount by Year", fig1)

# Query 2: App Opens by Device Brand
with col2:
    fig2, ax2 = plt.subplots(figsize=(6, 3))
    sns.barplot(data=datasets["q2"].head(10), x='brand', y='total_opens', hue='brand', palette='Purples', ax=ax2)
    ax2.set_title("App Opens by Device Brand")
    ax2.set_ylabel("Total Opens")
    ax2.set_xlabel("Brand")
    ax2.legend().set_visible(False)
    ax2.tick_params(axis='x', rotation=45)
    show_plot("2. App Opens by Device Brand", fig2)

# Query 3: Insurance Growth by State
with col1:
    fig3, ax3 = plt.subplots(figsize=(6, 3))
    sns.lineplot(data=datasets["q3"].head(10), x='state', y='total_value', marker='o', color='green', ax=ax3)
    ax3.set_title("Insurance Value Growth by State")
    ax3.set_ylabel("₹ Crores")
    ax3.set_xlabel("State")
    ax3.tick_params(axis='x', rotation=45)
    show_plot("3. Insurance Value Growth by State", fig3)

# Query 4: Transaction Share by State
with col2:
    fig4, ax4 = plt.subplots(figsize=(5, 5))
    ax4.pie(datasets["q4"]["total_transaction_value"].head(10),
            labels=datasets["q4"]["state"].head(10),
            autopct='%1.1f%%', startangle=140)
    ax4.set_title("Transaction Share by State")
    show_plot("4. Transaction Share by State", fig4)

# Query 5: Registered Users by State
with col1:
    fig5, ax5 = plt.subplots(figsize=(6, 3))
    sns.barplot(data=datasets["q5"].head(10), x='state', y='total_users', hue='state', palette='rocket', ax=ax5)
    ax5.set_title("Registered Users by State")
    ax5.set_ylabel("Users")
    ax5.set_xlabel("State")
    ax5.tick_params(axis='x', rotation=45)
    ax5.legend().set_visible(False)
    show_plot("5. Top 10 States by Registered Users", fig5)

# Query 6: Insurance Transactions by State
with col2:
    fig6, ax6 = plt.subplots(figsize=(6, 3))
    sns.barplot(data=datasets["q6"].head(10), x='state', y='insurance_transactions', hue='state', palette='crest', ax=ax6)
    ax6.set_title("Insurance Transactions by State")
    ax6.set_ylabel("Count")
    ax6.set_xlabel("State")
    ax6.tick_params(axis='x', rotation=45)
    ax6.legend().set_visible(False)
    show_plot("6. Top 10 States by Insurance Transactions", fig6)

# Query 7: Transaction Amount by District
with col1:
    ...
    ax7.set_title("Transaction Amount by District")
    ax7.set_ylabel("₹")
    ax7.set_xlabel("District")
    ax7.tick_params(axis='x', rotation=45)
    show_plot("7. Top 10 Districts by Transaction Amount", fig7)

# Query 8: Registered Users by District
with col2:
    ...
    ax8.set_title("Registered Users by District")
    ax8.set_ylabel("Users")
    ax8.set_xlabel("District")
    ax8.tick_params(axis='x', rotation=45)
    ax8.legend().set_visible(False)
    show_plot("8. Top 10 Districts by Registered Users", fig8)

# Query 9: Insurance Transactions by Pincode
with col1:
    ...
    ax9.set_title("Insurance Transactions by Pincode")
    ax9.set_ylabel("Transactions")
    ax9.set_xlabel("Pincode")
    ax9.tick_params(axis='x', rotation=45)
    ax9.legend().set_visible(False)
    show_plot("9. Top 10 Pincodes by Insurance Transactions", fig9)
